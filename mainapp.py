...
page = st.sidebar.selectbox("Choose a page", [PAGE_HOME, PAGE_PAGE1, PAGE_PAGE2,PAGE_PAGE3])
def home_page():
    st.title("Home Page")
...
def page3():
    st.title("Load a csv to file to predict output")
...
# Load the tokenizer used during model training
    ...
    def clean_text(text):
        ...
        # Example: Remove special characters, lowercasing, etc.
        cleaned_text = text.lower()
        cleaned_text = cleaned_text.replace('!', ' ')  # Replace exclamation marks with spaces
        return cleaned_text
    ...
    def main():
        ...
        # text_data = df['Tweet'].astype(str)
            ...
            wordcloud = generate_wordcloud(df['Tweet'])
            st.pyplot(wordcloud)
